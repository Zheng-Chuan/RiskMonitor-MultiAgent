...
@pytest.mark.asyncio
async def test_mcp_client_can_run_monitoring_flow_and_read_service_metrics() -> None:
    ...
    try:
        ...
        async with stdio_client(params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                assert "request_id" in payload2
                assert isinstance(payload2.get("summary"), dict)
                assert "uptime_seconds" in payload2["summary"]
