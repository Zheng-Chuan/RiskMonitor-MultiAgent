...
async def main():
    parser = argparse.ArgumentParser(prog="multiagent_eval")
    parser.add_argument("--run-tag", type=str, default="multiagent_eval", help="运行标识")
    parser.add_argument("--num-cases", type=int, default=10, help="使用前 N 个用例")
    args = parser.parse_args()

    bench_path = _PROJECT_ROOT / "eval" / "benchmarks" / "multiagent_cases.jsonl"
    cases = load_benchmark_cases(str(bench_path))
    
    print(f"=" * 80)
    print(f"多 Agent 协作系统评估")
    print(f"=" * 80)
