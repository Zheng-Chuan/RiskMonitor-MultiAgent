...
- [x] Checkpoint 14.6.2 企业通讯平台适配器 → **已回退 (2026-06-27)**
  - ~~实现项: 实现至少 2 个企业通讯平台适配器 (企业微信, Slack 或 Telegram). 支持告警推送, 审批交互, 状态查询三种消息模式.~~
  - **状态**: Slack 和 WeChat Work 适配器已从代码库移除, 核心 GatewayAdapter 抽象层保留.
  - 原因: 企业通讯平台适配器功能不再需要.
...
- ~~实现至少 2 个企业通讯平台适配器~~ (已回退 2026-06-27, 核心抽象层保留)
- 3 个金融场景调度模板端到端跑通

## 交付物清单

- [x] 代码：CronManager, 调度预算隔离, GatewayAdapter (核心抽象层保留, Slack/WeChat 适配器已移除)
