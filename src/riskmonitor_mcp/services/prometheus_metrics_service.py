...
def generate_prometheus_metrics() -> str:
    ...
    for tool_name in _request_latency_sum:
        if _request_latency_count[tool_name] > 0:
            avg_latency = _request_latency_sum[tool_name] / _request_latency_count[tool_name]
    ...
    for tool_name in _request_count:
        error_count = _error_count.get(tool_name, 0)
        request_count = _request_count[tool_name]
        if request_count > 0:
...
def reset_metrics() -> None:
    """重置所有指标(用于测试)"""
    global _request_count, _request_latency_sum, _request_latency_count, _error_count, _start_time
    _request_count = {}
