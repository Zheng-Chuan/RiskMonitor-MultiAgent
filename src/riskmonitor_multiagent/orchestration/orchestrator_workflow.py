...
class OrchestratorState(TypedDict):
    ...
    # 执行产物
    artifacts: Annotated[dict[str, Any], _reduce_dict]
    receipts: Annotated[list[dict[str, Any]], _reduce_list]
    pending_questions: Annotated[list[dict[str, Any]], _reduce_list]
    ...
    # 错误和统计
    errors: Annotated[list[str], _reduce_list]
    tokens_accumulated: int

    # 内部控制字段
    revision_count: int  # 计划修订计数
    exec_round: int  # 执行轮次计数
...
def _add_usage(state: OrchestratorState, usage: Any) -> None:
...
def _ensure_evidence_refs(out: dict[str, Any] | None, default_fields: list[str]) -> None:
    """若 out 的 evidence 无任何引用，则用 default_fields 回填 evidence.fields."""
...
async def intent_node(state: OrchestratorState) -> Command:
    """意图识别节点."""
    task = state.get("task", {})
    ...
    md = build_intent_metadata(task=task, policy_version=get_policy_version(), prompt_version=PROMPT_VERSION_INTENT)

    started = time.monotonic()
    intent_res = await intent_agent.recognize(task=task, metadata=md)
    observe_ms("rm_pipeline_node", (time.monotonic() - started) * 1000.0, labels={"node": "intent"})
    ...
    return Command(update=state, goto="plan_node")


async def plan_node(state: OrchestratorState) -> Command:
    """计划制定与评审节点（支持修订循环）."""
    task = state.get("task", {})
    session_id = str(task.get("session_id") or "default")
    run_id = state["run_id"]
    ...
    if approval.get("required") and not approval.get("approved"):
        # 需要人工审批，直接结束（不进入 finalize_node）
        return Command(update={**state, "should_stop": True}, goto=END)

    if critic.get("ok") is True:
        # 评审通过，进入执行
        return Command(update=state, goto="execute_node")

    if rev >= max_rev:
        # 达到最大修订次数，进入执行
        return Command(update={**state, "revision_count": rev + 1}, goto="execute_node")

    # 继续修订
    return Command(update={**state, "revision_count": rev + 1}, goto="plan_node")


async def execute_node(state: OrchestratorState) -> Command:
    """计划执行节点."""
    task = state.get("task", {})
    session_id = str(task.get("session_id") or "default")
    run_id = state["run_id"]
        ...
        if kind == "delegate":
            ...
            if target == "system_engineer":
                ...
                if isinstance(eng_res, Exception):
                    errors.append(f"engineer_error:{eng_res}")
                ...
                if isinstance(ana_res, Exception):
                    errors.append(f"analyst_error:{ana_res}")
                ...
                errors.append(f"unknown_target_agent:{target}")
            ...
            errors.append(f"unknown_step_kind:{kind}")
    ...
    # 检查是否需要重新规划
    if exec_round + 1 < max_rounds:
        next_plan = state.get("orchestrator_plan", {})
        next_steps = next_plan.get("plan_steps", [])
        has_executable = any(isinstance(s, dict) and s.get("kind") in {"delegate", "tool_call", "ask_human", "stop"} for s in next_steps)
        if has_executable and not next_plan.get("degraded"):
            return Command(update=state, goto="plan_node")

    return Command(update=state, goto="finalize_node")


async def finalize_node(state: OrchestratorState) -> Command:
    """最终汇总与评审节点."""
    task = state.get("task", {})
    session_id = str(task.get("session_id") or "default")
    run_id = state["run_id"]
    ...
    try:
        ...
        await mem.upsert_run_summary(run_id=run_id, summary=summary)
    except Exception:
        pass
...
def _build_quality_summary(state: OrchestratorState) -> dict[str, Any]:
    """构建质量汇总."""
    plan = state.get("orchestrator_plan", {})
    final_out = state.get("orchestrator_final", {})
    critic_plan = state.get("critic_plan", {})
...
def _build_step_trace(state: OrchestratorState) -> list[dict[str, Any]]:
    """构建步骤追踪."""
    plan = state.get("orchestrator_plan", {})
    steps = plan.get("plan_steps", [])
    artifacts = state.get("artifacts", {})
...
def _build_final_output(state: OrchestratorState) -> dict[str, Any]:
    """构建最终输出."""
    task = state.get("task", {})
...
def _create_workflow_graph() -> StateGraph:
    """创建 LangGraph 工作流图."""
...
def _get_compiled_graph():
    ...
    if _COMPILED_GRAPH is None:
        _COMPILED_GRAPH = _create_workflow_graph()
