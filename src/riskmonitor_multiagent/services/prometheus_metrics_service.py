...
def record_request(tool_name: str, latency_ms: float, is_error: bool = False) -> None:
    ...
    # 错误计数
    if is_error:
        _store.error_count[tool_name] = _store.error_count.get(tool_name, 0) + 1
...
def generate_prometheus_metrics() -> str:
    ...
    lines.append(f"process_start_time_seconds {_store.start_time}")
    lines.append("")

    # 进程运行时间
    uptime = time.time() - _store.start_time
    ...
    for tool_name, count in _store.request_count.items():
    ...
    for tool_name, latency_sum in _store.request_latency_sum.items():
        latency_count = _store.request_latency_count.get(tool_name, 0)
        if latency_count > 0:
    ...
    for tool_name, count in _store.error_count.items():
    ...
    for tool_name, request_count in _store.request_count.items():
        error_count = _store.error_count.get(tool_name, 0)
        if request_count > 0:
...
def get_metrics_summary() -> Dict[str, Any]:
    ...
    """
    summary = {
        "uptime_seconds": time.time() - _store.start_time,
        "tools": {}
    }

    for tool_name, request_count in _store.request_count.items():
        error_count = _store.error_count.get(tool_name, 0)
        latency_sum = _store.request_latency_sum.get(tool_name, 0.0)
        latency_count = _store.request_latency_count.get(tool_name, 0)
